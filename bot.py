...
class AIClient:
    ...
    def _ask_deepseek(self, prompt, max_tokens):
        ...
        payload = {
            "model": "deepseek-chat",
            "messages": [
                {{"role": "system", "content": "Ты — профессиональный астролог с 25-летним опытом. Используй нейтральные обращения (человек/партнёр), избегай указания пола. Ты обязан всегда давать полный ответ. Если данных недостаточно — дай общие рекомендации. Никогда не отвечай отказом."},
...
async def compat_cmd(update, ctx):
    await update.message.reply_text("💑 *Совместимость*\n\nВведите два знака: *Овен Телец*", parse_mode='Markdown')
...
def main():
    ...
    app.add_handler(CommandHandler('natal', natal_cmd))
