...
async def support_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Стартовое сообщение бота поддержки"""
    await update.message.reply_text(
        "💬 *Поддержка АстроБота*\n\n"
        "Напишите ваш вопрос, и я передам его астрологу.\n"
        "Ответ придёт в течение нескольких часов.",
        parse_mode='Markdown'
    )
...
async def forward_to_admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text += f"👤 *{user.full_name}* (@{user.username or 'нет username'})\n"
    text += f"🆔 ID: `{user.id}`\n\n"
    text += f"💬 *Сообщение:*\n{msg.text or '[не текст]'}\n\n"
    text += f"_Чтобы ответить, напишите:_\n`/reply {user.id} ваш ответ`"
    ...
    await msg.reply_text("✅ *Ваше сообщение отправлено!*\nАстролог ответит в ближайшее время.", parse_mode='Markdown')
...
async def reply_to_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(args) < 3:
        await update.message.reply_text("❌ Формат: `/reply ID текст`", parse_mode='Markdown')
        return
...
def main():
    ...
    # Запускаем бота поддержки в отдельном потоке, если есть токен
    if SUPPORT_TOKEN:
