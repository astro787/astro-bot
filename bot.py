...
8. 6-8 предложений. Конкретно, без общих фраз."""
...
SYSTEM_PROMPT_COMPAT = "Ты — астролог. Нейтральные обращения. Отвечай всегда. Дай краткую совместимость. Упоминай конкретные знаки.Завершай каждое предложение.Не обрывай."
...
async def forecast_cmd(update, ctx):
    ...
    if uid in users and 'sign' in users[uid]:
        ...
        await update.message.reply_text("🔮 *Прогноз ИИ Введите свои данные по образцу*\nВведите: `ДД.ММ.ГГГГ ЧЧ:ММ Город`", parse_mode='Markdown')
...
async def btn(update, ctx):
    ...
    if d == 'forecast':
        if uid in users and 'sign' in users[uid]:
            ...
            await q.edit_message_text("🔮 *Прогноз ИИ Введите свои данные по примеру ниже *\n\n📝 `15.05.1990 14:30 Москва`\n📝 `15.05.1990 Москва`",
                reply_markup=InlineKeyboardMarkup([
